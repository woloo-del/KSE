...
- **KSE-049 — Niezależna walidacja tożsamości profili i kosztu wyjątków** (P0, W toku). Zweryfikować tożsamość i zakres czasowy 38 profili z bezpośrednią etykietą stacji w portalu PSE; pozostałe konteksty oraz aliasy rozstrzygnąć osobno. Warstwa kontekstu nie zatwierdza tożsamości ani napięcia aktualnej infrastruktury.
...
- Odpowiedzialność: Codex.
- Następny krok: Zweryfikować tożsamość i zakres czasowy 38 profili z bezpośrednią etykietą stacji w portalu PSE; pozostałe konteksty oraz aliasy rozstrzygnąć osobno. Warstwa kontekstu nie zatwierdza tożsamości ani napięcia aktualnej infrastruktury.
...
- Termin docelowy: nie ustalono.
- Zakończono: nie zakończono.
- Dowody/kontekst: [docs/21_scalability_benchmark.md](docs/21_scalability_benchmark.md), [docs/23_pse_identity_audit.md](docs/23_pse_identity_audit.md), [docs/24_pse_bulk_investment_index.md](docs/24_pse_bulk_investment_index.md), [tests/test_pse_mention_context.py](tests/test_pse_mention_context.py), [data/reference/pse_mention_context_2026-09-18_v1.json](data/reference/pse_mention_context_2026-09-18_v1.json).
...
### KSE-056 — Transformacja CRS i kontrola kompletności pojedynczej odpowiedzi działek

- Odpowiedzialność: Codex.
- Następny krok: Rozszerzyć o kontrolowany import pełnego bufora; status gotowej odpowiedzi nie zatwierdza stacji ani praw do danych.
- Kryterium: Jawne osie i CRS, hash źródła, kontrola BBOX względem bufora, liczby dopasowań i kolejnej strony; brak wyniku przy ograniczonej próbce.
- Nieukończone zależności: brak.
- Ryzyko: brak dodatkowej uwagi w rejestrze.
- Termin docelowy: nie ustalono.
- Zakończono: 2026-09-19.
